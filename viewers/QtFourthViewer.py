...
class QtFourthViewer(QtViewer):
    ...
    def update_mode(self, index):
        if index == 0:  # Outline Mode
            self.viewer.hide_oblique()
            # The outline is shown/hidden via the organ detection widget
        elif index == 1:  # Oblique Plane Mode
            self.viewer.hide_outline()
            self.viewer.show_oblique()
